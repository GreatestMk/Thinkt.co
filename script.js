...
function initTypingEffect(spanId, options) {
  ...
  const type = () => {
    ...
    if (!deleting) {
      charIndex++;
      if (charIndex >= current.length + (options.extraPause || 6)) {
        deleting = options.loop;
      }
    } else {
      charIndex--;
      if (charIndex <= 0) {
    ...
    const delay = deleting
      ? (options.deleteSpeed || 120)
      : (options.typeSpeed || 180);
...
// About section typing (looping, default speed)
initTypingEffect('hero-typing', {
  loop: true,
  typeSpeed: 140,
  deleteSpeed: 80,
  extraPause: 6
...
// Main hero title typing (slower, no delete)
initTypingEffect('hero-typing-main', {
  loop: false,
  typeSpeed: 220,
  deleteSpeed: 140,
  extraPause: 12
